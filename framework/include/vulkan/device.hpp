...
namespace vks {
    ...
    class VulkanPhysicalDevice::Selector {
        public:
            ...
            Selector& with_selector_function(int (*selector)(const Properties&, const Features&));
            
            Selector& with_debug_name(const char* name);
            
        private:
            NODISCARD Properties query_properties(VkPhysicalDevice handle) const;
            NODISCARD Features query_features(VkPhysicalDevice handle) const;
