...
////    }
//
//    VulkanDevice::Builder::Builder(std::shared_ptr<VulkanInstance> instance) : fp_vk_enumerate_physical_devices_(reinterpret_cast<typeof(fp_vk_enumerate_physical_devices_)>(detail::vk_get_instance_proc_addr(*instance, "vkEnumeratePhysicalDevices"))),
...
//    }
//
//    std::shared_ptr<VulkanDevice> VulkanDevice::Builder::build() const {
//        VkResult result;
//
...
//            if (!verify_device_validation_layer_support(current.device)) {
//                continue;
//            }
//
//            // Verify that given device supports all requested device extensions.
//            if (!verify_device_extension_support(current.device)) {
//                continue;
//            }
//
//            // Verify that given device properties meet requested requirements.
//            fp_vk_get_physical_device_properties_(current.device, &current.properties);
////            if (!verify_device_properties(context.properties)) {
////                continue;
////            }
//
//            // Verify that requested device features are supported by the given device.
//            fp_vk_get_physical_device_features_(current.device, &current.features);
//            if (!verify_device_feature_support(current.features)) {
//                continue;
//            }
//
//            // Verify that a valid queue family configuration exists on this device.
//            u32 queue_family_count = 0u;
//            fp_vk_get_physical_device_queue_family_properties_(current.device, &queue_family_count, nullptr);
//
//            current.queue_family_properties.resize(queue_family_count);
//            fp_vk_get_physical_device_queue_family_properties_(current.device, &queue_family_count, current.queue_family_properties.data());
//
//            current.queue_families = select_device_queue_families(current.device, current.queue_family_properties);
//
//            if (!verify_device_queue_family_support(current.queue_families)) {
//                continue;
//            }
//
//            int current_device_score = 0;//selector_(device_properties, device_features, device_queue_families);
//
...
//        std::unordered_set<u32> unique_queue_family_indices {
//            static_cast<u32>(std::get<GraphicsQueueFamily>(selected.queue_families).index),
//            static_cast<u32>(std::get<PresentationQueueFamily>(selected.queue_families).index),
//            static_cast<u32>(std::get<ComputeQueueFamily>(selected.queue_families).index),
//            static_cast<u32>(std::get<TransferQueueFamily>(selected.queue_families).index)
//        };
//
...
//        };
//
//        result = fp_vk_create_device_(selected.device, &device_create_info, nullptr, &selected.handle);
//    }
//
...
//    }
//
//    std::tuple<VulkanDevice::Builder::GraphicsQueueFamily, VulkanDevice::Builder::PresentationQueueFamily, VulkanDevice::Builder::ComputeQueueFamily, VulkanDevice::Builder::TransferQueueFamily> VulkanDevice::Builder::select_device_queue_families(VkPhysicalDevice device_handle, const std::vector<VkQueueFamilyProperties>& queue_families) const {
//        i32 invalid = -1;
...
//        }
//
//        return std::make_tuple(graphics_queue_family, presentation_queue_family, compute_queue_family, transfer_queue_family);
//    }
//
//
//    bool VulkanDevice::Builder::verify_device_queue_family_support(const std::tuple<GraphicsQueueFamily, PresentationQueueFamily, ComputeQueueFamily, TransferQueueFamily>& queue_families) const {
//        i32 invalid = -1;
//
//        const GraphicsQueueFamily& graphics_queue_family = std::get<GraphicsQueueFamily>(queue_families);
//        if (graphics_queue_family.index == invalid) {
//            // Found no queue families that support graphics operations.
//            return false;
//        }
//
//        const PresentationQueueFamily& presentation_queue_family = std::get<PresentationQueueFamily>(queue_families);
//        if (presentation_queue_family.index == invalid) {
//            // Found no queue families that support presentation operations.
//            return false;
//        }
//
//        const ComputeQueueFamily& compute_queue_family = std::get<ComputeQueueFamily>(queue_families);
//        if (compute_queue_family.index == invalid) {
//            // Found no queue families that support compute operations.
//            return false;
//        }
//
//        const TransferQueueFamily& transfer_queue_family = std::get<TransferQueueFamily>(queue_families);
//        if (transfer_queue_family.index == invalid) {
//            // Found no queue families that support transfer operations.
//            return false;
//        }
//
//        return true;
